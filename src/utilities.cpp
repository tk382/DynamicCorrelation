...
//' @export
// [[Rcpp::export]]
double get_score_w_c(const arma::mat x,
                   const arma::vec w,
                   const arma::vec v) {
  int n = w.size();
  arma::mat B = orth(x) * sqrt(n);
  double sigw = sum(w%w)/n;
  double sigv = sum(v%v)/n;
  arma::vec grad = get_grad_c(sigw, sigv, w, v, B);
  double fisherinf = const_c(sigw, sigv);
...
//' @export
// [[Rcpp::export]]
arma::vec get_score_W_c(const arma::mat x,
                     const arma::mat W,
                     const arma::mat V) {
  int n = x.n_rows;
  arma::mat B = orth(x) * sqrt(n);
  ...
  for (int i=0; i < K; ++i){
    out(i) = get_score_w_c(B, W.col(i), V.col(i));
  }
  return out;
}
...
//' @export
// [[Rcpp::export]]
double get_degree_w_c(const arma::mat x,
                      const arma::mat W,
                      const arma::mat V){
  //degree statistic for i'th gene based on matrix Y
  arma::mat B = orth(x) * sqrt(x.n_rows);
  ...
  double d = 0;
  for (int i=0; i < K; ++i){
    d = d + get_score_w_c(B, W.col(i), V.col(i));
  }
  return d;
}
